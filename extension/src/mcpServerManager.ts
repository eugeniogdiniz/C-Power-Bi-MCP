...
export class MCPServerManager implements vscode.McpServerDefinitionProvider<vscode.McpStdioServerDefinition> {
  ...
  private getServerExecutablePath(): string {
    ...
    switch (platform) {
      ...
      case 'linux':
        ...
        break;
      default:
        this.logger.error(`Unsupported platform: ${platform}. Only Windows, macOS, and Linux are supported.`);
        throw new Error(`Unsupported platform: ${platform}. Only Windows, macOS, and Linux are supported.`);
    }
    ...
    // Ensure execute permissions for macOS and Linux
  ...
  private ensureExecutePermissions(filePath: string): void {
    try {
      ...
      this.logger.info(`Execute permissions set for executable: ${filePath}`);
    } catch (error) {
